	...
	script.Parent.Upgrader.Touched:Connect(function(Hit)
		if Hit.Parent and Hit:IsDescendantOf(Tycoon) then
			if Hit.Parent == Tycoon.Ores then
				...
				local Tag2 = ItemLib.GetTag(Hit,"Gyrosphere")
				if Tag2 and Tag2.Value >= 1 then
...
if Server then
	script.Upgrade.OnServerEvent:Connect(function(Player,Ore)
		...
		local Tag2 = ItemLib.GetTag(Ore,"Gyrosphere")
		if Tag2 and Tag2.Value >= 1 then
			return
		end
		if Tag then
		...
		if Tag2 then
			ItemLib.UpdateTag(Ore,"Gyrosphere",Tag2.Value + 1)
		else
			Tag2 = ItemLib.CreateTag(Ore,"Gyrosphere")
			ItemLib.UpdateTag(Ore,"Gyrosphere",1)
		end
		Ore.Cash.Value = (Ore.Cash.Value * 8)
	end)
end
