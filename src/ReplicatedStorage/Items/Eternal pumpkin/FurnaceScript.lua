	...
	script.Parent.Furnace.Touched:Connect(function(Hit)
		if Hit.Parent ~= nil and Hit:IsDescendantOf(Tycoon) then
			if Hit:FindFirstChild("Cash") ~= nil then
				...
				game.ReplicatedStorage.ProcessOre:FireServer(Hit,script.Parent.Furnace,Color3.fromRGB(213,115,61),"rbxassetid://298181829")
...
if Server then
	script.HandleCash.OnServerEvent:Connect(function(Player,Ore)
		if Ore:FindFirstChild("Cash") and Ore:IsDescendantOf(Tycoon) then
			local Multi = 6.8
			if Ore.Tags:FindFirstChild("Spook") then
				Multi = Multi * (Ore.Tags.Spook.Value + 0.6)
			else
				Multi = Multi * 1.6
			end
			Ore.Cash.Value = (Ore.Cash.Value * Multi)
		end
	end)
end
